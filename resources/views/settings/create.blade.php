...
@section('content')
    ...
    <div class="min-h-screen bg-gray-950 text-gray-100 pb-12">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pt-10">
            ...
            <div class="flex items-center justify-between mb-10">
                <div>
                    <h1 class="text-3xl font-bold tracking-tight">Создать новую группу настроек</h1>
                    <p class="mt-2 text-gray-400">Группа + вложенные параметры (ключ = boolean)</p>
                </div>
            ...
            <form method="POST" action="{{ route('settings.store') }}" class="space-y-8">
                ...
                <div class="bg-gray-900/70 backdrop-blur-xl border border-gray-800 rounded-2xl p-8 shadow-xl shadow-purple-950/20">
                    ...
                    <div id="settings-container" class="space-y-4">
                        @if(old('keys') && is_array(old('keys')))
                            @foreach(old('keys') as $index => $key)
                                <div class="setting-row flex flex-col sm:flex-row sm:items-end gap-4 bg-gray-800/40 p-4 rounded-lg border border-gray-700">
                                    <div class="flex-1">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            let index = {{ old('keys') ? count(old('keys')) : 0 }};
